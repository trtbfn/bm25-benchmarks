...
and throughput statistics are persisted to disk for later analysis.
"""
...
_STOPWORD_PRESETS = {
    "en": tuple(sorted(bm25s_tokenization.STOPWORDS_EN)),
    "english": tuple(sorted(bm25s_tokenization.STOPWORDS_EN)),
}
    ...
    def search(self, query: str, k: int = 10) -> List[Tuple[int, float, str]]:
        ...
        for term in unique_terms:
            ...
            if self.sidf_mode == "cms" and self.cms is not None:
                df_estimate = max(1.0, self.cms.query(term))
                idf = math.log((num_docs - df_estimate + 0.5) / (df_estimate + 0.5))
            elif self.sidf_mode == "cs" and self.cs is not None:
                df_estimate = max(1.0, self.cs.query(term))
                idf = math.log((num_docs - df_estimate + 0.5) / (df_estimate + 0.5))
            else:
                idf = math.log((num_docs - df + 0.5) / (df + 0.5))
            if self.clamp_idf_nonneg and idf < 0.0:
                idf = 0.0
            ...
            for idx, doc_id in enumerate(doc_ids):
                scores[int(doc_id)] = scores.get(int(doc_id), 0.0) + float(increments[idx])
        ...
        if len(items) > k:
            indices = np.argpartition([-score for _, score in items], kth=k - 1)[:k]
            top = [items[i] for i in indices]
            top.sort(key=lambda item: -item[1])
        else:
            top = sorted(items, key=lambda item: -item[1])

        return [(doc_id, float(score), self.doc_names[doc_id]) for doc_id, score in top[:k]]
    ...
    try:
        for start, end in batched_indices(len(query_texts), batch_size):
            ...
            if executor is not None:
                futures = [executor.submit(model.search, query, top_k) for query in batch_queries]
                batch_results = [future.result() for future in futures]
            else:
                batch_results = [model.search(query, top_k) for query in batch_queries]

            for qid, hits in zip(batch_ids, batch_results):
                results[qid] = {
                    (model.doc_names[doc_id] if isinstance(doc_id, int) else str(doc_id)): float(score)
...
def main(
    *,
    ...
    results_dict = retrieve_in_batches(
        model,
        query_ids,
        query_texts,
